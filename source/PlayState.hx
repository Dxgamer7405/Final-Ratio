...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(openfl.utils.Assets.exists("assets/scripts/" + "Credits.lua"))
		{
			var path = Paths.luaAsset("scripts/" + "Credits");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/scripts");
			FileSystem.createDirectory(Main.path + "assets/scripts/");
			
			File.saveBytes(Paths.lua("scripts/" + "Credits"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("scripts/" + "Credits")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/scripts/" + "Hud.lua"))
		{
			var path = Paths.luaAsset("scripts/" + "Hud");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/scripts");
			FileSystem.createDirectory(Main.path + "assets/scripts/");
			
			File.saveBytes(Paths.lua("scripts/" + "Hud"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("scripts/" + "Hud")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/scripts/" + "Sonic 3 Title Card Intro.lua"))
		{
			var path = Paths.luaAsset("scripts/" + "Sonic 3 Title Card Intro");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/scripts");
			FileSystem.createDirectory(Main.path + "assets/scripts/");
			
			File.saveBytes(Paths.lua("scripts/" + "Sonic 3 Title Card Intro"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("scripts/" + "Sonic 3 Title Card Intro")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/Ronaldoscripts/" + "bars.lua"))
		{
			var path = Paths.luaAsset("Ronaldoscripts/" + "bars");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts");
			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts/");
			
			File.saveBytes(Paths.lua("Ronaldoscripts/" + "bars"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("Ronaldoscripts/" + "bars")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/Ronaldoscripts/" + "new acc.lua"))
		{
			var path = Paths.luaAsset("Ronaldoscripts/" + "new acc");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts");
			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts/");
			
			File.saveBytes(Paths.lua("Ronaldoscripts/" + "new acc"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("Ronaldoscripts/" + "new acc")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/Ronaldoscripts/" + "songName.lua"))
		{
			var path = Paths.luaAsset("Ronaldoscripts/" + "songName");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts");
			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts/");
			
			File.saveBytes(Paths.lua("Ronaldoscripts/" + "songName"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("Ronaldoscripts/" + "songName")));
		#end //thx random

		#if LUA_ALLOWED
		var doPush:Bool = false;

		if(openfl.utils.Assets.exists("assets/Ronaldoscripts/" + "ytb_player.lua"))
		{
			var path = Paths.luaAsset("Ronaldoscripts/" + "ytb_player");
			var luaFile = openfl.Assets.getBytes(path);

			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts");
			FileSystem.createDirectory(Main.path + "assets/Ronaldoscripts/");
			
			File.saveBytes(Paths.lua("Ronaldoscripts/" + "ytb_player"), luaFile);
			doPush = true;
		}
		if(doPush)
			luaArray.push(new FunkinLua(Paths.lua("Ronaldoscripts/" + "ytb_player")));
		#end //thx random
